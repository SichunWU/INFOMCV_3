...
def firstRun(filename):
    ...
    mtx_elem = ET.SubElement(root, 'mtx')
    dist_elem = ET.SubElement(root, 'dist')
    rvecs_elem = ET.SubElement(root, 'rvecs')
    tvecs_elem = ET.SubElement(root, 'tvecs')
    mtx_elem.text = np.array2string(mtx)
    dist_elem.text = np.array2string(dist)
    rvecs_str = "["
    for r in rvecs:
        rvecs_str += np.array2string(r, separator=" ") + " "
    rvecs_elem.text = rvecs_str[:-1] + "]"
    tvecs_str = "["
    for t in tvecs:
        tvecs_str += np.array2string(t, separator=" ") + " "
    tvecs_elem.text = tvecs_str[:-1] + "]"

    xml_data = ET.tostring(root, encoding='utf-8', method='xml')
    newfile = filename.replace('capture/*.jpg', 'config.xml')
